...
case class PrincessFunctionInvocation(funDef: FunDef, args: Seq[IExpression])

class PrincessFunctionTemplate private (
  solver: PrincessSolver,
  val funDef: FunDef,
  ...
  val princessActivatingBool = solver.idToPrincessId(activatingBool).asInstanceOf[IFormula]

  private val princessFunDefArgs = funDef.args.map(ad => solver.idToPrincessId(ad.id))

  private val zippedCondVars = condVars.map(id => (id, solver.idToPrincessId(id)))
  private val zippedExprVars = exprVars.map(id => (id, solver.idToPrincessId(id)))
  private val zippedFunDefArgs = funDef.args.map(_.id) zip princessFunDefArgs

  val idToPrincessIds: Map[Identifier, IExpression] = {
    Map(activatingBool -> princessActivatingBool) ++
      zippedCondVars ++
      zippedExprVars ++
      zippedFunDefArgs
  }

  val asPrincessClauses: Seq[IExpression] = asClauses.map {
    //solver.toPrincessFormula(_, idToZ3Ids).getOrElse(sys.error("Could not translate to z3. Did you forget --xlang?"))
    solver.toPrincessFormula(_).getOrElse(sys.error("Could not translate to z3. Did you forget --xlang?"))
  }
  ...
  val princessBlockers: Map[IExpression, Set[PrincessFunctionInvocation]] = blockers.map {
    case (b, funs) =>
      //(idToZ3Ids(b) -> funs.map(fi => PrincessFunctionInvocation(fi.funDef, fi.args.map(solver.toPrincessFormula(_, idToZ3Ids).get))))
      (idToPrincessIds(b) -> funs.map(fi => PrincessFunctionInvocation(fi.funDef, fi.args.map(solver.toPrincessFormula(_).get))))
  }

  // We use a cache to create the same boolean variables.
  private val cache: MutableMap[Seq[IExpression], Map[IExpression, IExpression]] = MutableMap.empty

  def instantiate(aVar: IFormula, args: Seq[IExpression]): (Seq[IExpression], Map[IExpression, Set[PrincessFunctionInvocation]]) = {   
    assert(args.size == funDef.args.size)

    // The "isRealFunDef" part is to prevent evaluation of "fake"
    // function templates, as generated from PrincessSolver.
    /*
     * NEED TO CHANGE
     * 
      ...
      if(ga.forall(_.isDefined)) {
        ...
        solver.getEvaluator.eval(invocation) match {
          case EvaluationResults.Successful(result) =>
            ...
            val z3Value      = solver.toPrincessFormula(result).get
    ...
    val (wasHit, baseIDSubstMap) = cache.get(args) match {
      case Some(m) => (true, m)
      case None =>
        val newMap: Map[IExpression, IExpression] =
          (zippedExprVars ++ zippedCondVars).map(p => p._2 -> solver.idToPrincessId(p._1)).toMap ++
            (princessFunDefArgs zip args)
        cache(args) = newMap
        (false, newMap)
    }

    val idSubstMap: Map[IExpression, IExpression] = baseIDSubstMap + (princessActivatingBool -> aVar)

    val (from, to) = idSubstMap.toSeq.unzip
    
    val newClauses = asPrincessClauses.map(solver.substitutePrincessFormula(_, from, to))
    val newBlockers = princessBlockers.map {
      case (b, funs) =>
        val bp = if (b.toString.equals(princessActivatingBool.toString)) {
          aVar
        } else {
          idSubstMap(b)
        }

        val newFuns = funs.map(fi => fi.copy(args = fi.args.map(solver.substitutePrincessFormula(_, from, to))))
...
object PrincessFunctionTemplate {
  val splitAndOrImplies = false

  def mkTemplate(solver: PrincessSolver, funDef: FunDef, isRealFunDef: Boolean = true): PrincessFunctionTemplate = {
    ...
    new PrincessFunctionTemplate(solver, funDef, activatingBool, Set(condVars.toSeq: _*), Set(exprVars.toSeq: _*), Map(guardedExprs.toSeq: _*),
      isRealFunDef)
  }
  
  //private val funDefTemplateCache : MutableMap[FunDef, PrincessFunctionTemplate] = MutableMap.empty
  //private val exprTemplateCache   : MutableMap[Expr  , PrincessFunctionTemplate] = MutableMap.empty

  def getTemplate(solver: PrincessSolver, funDef: FunDef): PrincessFunctionTemplate = {
    mkTemplate(solver: PrincessSolver, funDef, true)
  }

  def getTemplate(solver: PrincessSolver, body: Expr): PrincessFunctionTemplate = {
    ...
    mkTemplate(solver, fakeFunDef, false)
  }
